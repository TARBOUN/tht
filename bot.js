...
setInterval(() => {
  http.get(`http://quran23.glitch.me/`);
}, 280000);
// ============ Settings =================== //
const PREFIX = "8"; // برافيكس
const CMD = 'A'; //  امر تغشيل قائمة الاصوات
const GUILDID = '608318367656312864'; // اي دي السيرفر  
const CHANNELID = '701141541237948497'; // اي دي الروم
const ownerID = ["599317636928634957"]; // ايدي ادارة البوت او صاحب البوت .. 
// ============ Settings =================== //
...
🇦 عمرو دياب - ألبوم سهران 
🇧 عمرو دياب - ألبوم احلي واحلي 
🇨 اعمرو دياب - ألبوم كل حياتي 
⏹ لإيقاف ألالبوم 
🇩 عمرو دياب - ألبوم معدي الناس
🇪 عمرو دياب - ألبوم قمرين 
🇫 عمرو دياب - ألبوم ليلي نهاري 
...
collector1.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = ytdl("https://www.youtube.com/playlist?list=PLXLRsD4jOF1sWFjsxT5GjlGUkVvsCVAEe", { filter: 'audioonly' });
		...
		collector6.stop();
		collector7.stop();
		embed.setDescription(`<@${message.author.id}> **Album is Now On**`); //Hi
...
collector2.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = ytdl("https://www.youtube.com/playlist?list=PL_AvxXMO2-gYI61HXDL6_k1NJNguBZ2vw", { filter: 'audioonly' });
        const dispatcher = connnection.playStream(stream);
        dispatcher.on('end', () => voiceChannel.leave());
		collector1.stop();
		collector2.stop();
...
collector3.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = ytdl("https://www.youtube.com/playlist?list=PLXLRsD4jOF1sKdoqYn5OpMhiQSabA4E40", { filter: 'audioonly' });
        const dispatcher = connnection.playStream(stream);
        dispatcher.on('end', () => voiceChannel.leave());
		collector1.stop();
		collector2.stop();
		...
		collector6.stop();
		collector7.stop();
		embed.setDescription(`<@${message.author.id}> **Album is Now On**`);
		msg.edit(embed).then(msg.delete(5000));
      });
});
collector4.on('collect', r => {
	if (message.guild.voiceConnection) message.guild.voiceConnection.disconnect();
		...
		collector6.stop();
		collector7.stop();
		embed.setDescription(`<@${message.author.id}> **Album is Now Off**`);
		msg.edit(embed).then(msg.delete(5000));
});
collector5.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = ytdl("https://www.youtube.com/playlist?list=PLXLRsD4jOF1v3JH2cJ7M9ZKdMKqqIbPWU", { filter: 'audioonly' });
        const dispatcher = connnection.playStream(stream);
        dispatcher.on('end', () => voiceChannel.leave());
		collector1.stop();
		collector2.stop();
...
collector6.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = ytdl("https://www.youtube.com/playlist?list=PLZejDZO2nSZt_jlNk9AGtBPFqAeHoa9VG", { filter: 'audioonly' });
        const dispatcher = connnection.playStream(stream);
        dispatcher.on('end', () => voiceChannel.leave());
		collector1.stop();
		collector2.stop();
...
collector7.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = ytdl("https://www.youtube.com/playlist?list=PLqjy2IHnuqJsC8IkM8hAVZmUQ5FIJAcQQ", { filter: 'audioonly' });
        const dispatcher = connnection.playStream(stream);
        dispatcher.on('end', () => voiceChannel.leave());
		collector1.stop();
		collector2.stop();
